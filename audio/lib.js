function A_Audio(context) {
	this.context=context;
	this.recorder=undefined;
	this._gain=1;
	this.render=function(){
    	analyser.getByteFrequencyData(frequencyData);
    	for(i=0;i<frequencyData.length;i++){frequencyData[i]=(frequencyData[i]*100)/256;}
    	success(frequencyData);
    	requestAnimationFrame(render);
	};
}


A_Audio.prototype.record_live=function(error,success){
	if(!Recorder){
		error();
		return;
	}
	...
	navigator.getMedia({audio:true,video:false}, function(stream){
		...
		this.recorder=new Recorder(gainNode);
...
A_Audio.prototype.pause=function(){
	this.recorder.stop();
};


A_Audio.prototype.stop=function(error,success,createLink=false){
	if(!this.recorder){
		success();
		return;
	}
	this.recorder.stop();
	if(createLink)this.createDownloadLink(error,createLink);

};

A_Audio.prototype.createDownloadLink=function(error,output){
...
A_Audio.prototype.clear=function(){
	this.recorder.clear();
	this.recorder=undefined;
};

A_Audio.prototype.ouput_recorder_audio=function(output){
...
A_Audio.prototype.get_file_formdata=function(success){
	if(this.recorder){
		...
		return "Error! You might have cleared Before";
	}
	
};

A_Audio.prototype.get_liveData_amplitude=function(error,success,replay=false){
	analyser = this.context.createAnalyser();
    navigator.getMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;
    navigator.getMedia({audio:true,video:false},function(stream){
    	source = this.context.createMediaStreamSource(stream);
    	source.connect(analyser);
        if(replay)analyser.connect(this.context.destination);
        analyser.fftSize = 32;
        frequencyData = new Uint8Array(analyser.frequencyBinCount);
        function render(){
        	analyser.getByteFrequencyData(frequencyData);
        	mid1=(frequencyData[7] * 100) / 256;
        	mid2=(frequencyData[8] * 100) / 256;
        	mid=(mid1+mid2)/2;
        	success(mid);
        	requestAnimationFrame(render);
        }
...
A_Audio.prototype.get_spikes_live=function(error,success,count){
	analyser = this.context.createAnalyser();
	if(count<32 || count>2048 || count%2!==0)count=32;
	if(arguments.length!==3){
		console.error("Pass 3 arguments. error and success function and spikes count in power of 2 ");
		return;
	}
...
A_Audio.prototype.get_spikes_media=function(error,success,count,media){
	analyser = this.context.createAnalyser();
	if(count<32 || count>2048 || count%2!==0)count=32;
...
// recorder Object by https://github.com/mattdiamond/Recorderjs
